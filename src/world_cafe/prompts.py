...
def contribution_prompt(
    *,
    ...
    system = (
        ...
        "Always write visible output in English, even when the user request, background, notes, persona material, or prior content are in another language. "
...
def host_opening_prompt(
    *,
    ...
    if round_index <= 0:
        round_instruction = (
            f"This is Round 1 ({focus}).\n"
            "Focus on divergent observation: open the field of observation. If there is no table_memory, do not force sub-questions.\n"
        )
    elif round_index == 1:
        round_instruction = (
            f"This is Round 2 ({focus}).\n"
            "Focus on connections and tensions: use Round 1 table_memory to avoid settled consensus and ask about new shifts, minority views, stakeholder tensions, or mechanism tensions.\n"
            "End with 2-3 brief open questions that can directly start discussion. Keep each question to one sentence."
        )
    else:
        round_instruction = (
            f"This is Round {round_index + 1} ({focus}).\n"
            "Focus on problem reframing: use prior table_memory to ask whether the original problem should be rewritten, which unresolved tension could become a design opportunity, and which assumption deserves testing.\n"
            "End with 2-3 brief open questions that can directly start discussion. Keep each question to one sentence."
        )
    user = (
        ...
        f"{round_instruction}\n\n"
        "When sub-questions are useful, use only 2-3 brief open questions that can directly start discussion.\n"
        "Opening template:\n"
        ...
        + (
            ...
            "- [Question 3, one sentence, no extra setup]\n"
            if round_index > 0
            else "Return only one brief opening phrase in under 10 English words, without extra questions.\n"
        )
    )
    return system, user
...
def host_synthesis_prompt(
    *,
    ...
    system = (
        ...
        "Always write JSON values in English, even when the user request, background, notes, persona material, or prior content are in another language. "
